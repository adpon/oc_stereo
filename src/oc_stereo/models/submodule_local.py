...
class matchshifted(nn.Module):
    ...
    def forward(self, left, right, shift):
        ...
        shifted_left = F.pad(torch.index_select(left, 3, Variable(
...
class disparityregression(nn.Module):
    def __init__(self, max_disp, min_disp, disp_range):
        super(disparityregression, self).__init__()
        self.disp = Variable(
            torch.Tensor(
                np.reshape(
                    np.array(
                        range(min_disp, max_disp)), [
                        1, disp_range, 1, 1])).cuda(), requires_grad=False)
...
class feature_extraction(nn.Module):
    def __init__(self):
        ...
        self.lastconv = nn.Sequential(
            convbn(
                320, 128, 3, 1, 1, 1), nn.ReLU(
                inplace=True), nn.Conv2d(
                128, 32, kernel_size=1, padding=0, stride=1, bias=False))
...
class Decoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.firstconv = nn.Sequential(convbn(32, 32, 3, 1, 1, 1),
                                       nn.ReLU(inplace=True),
                                       convbn(32, 32, 3, 1, 1, 1),
                                       nn.ReLU(inplace=True),
                                       convbn(32, 32, 3, 1, 1, 1),
        ...
        self.secondconv = nn.Sequential(convbn(32, 32, 3, 1, 1, 1),
                                        nn.ReLU(inplace=True),
                                        convbn(32, 32, 3, 1, 1, 1),
                                        nn.ReLU(inplace=True),
                                        convbn(32, 32, 3, 1, 1, 1),
                                        nn.ReLU(inplace=True))
        self.outputconv = nn.Conv2d(32, 1, 3, 1, 1, 1)
